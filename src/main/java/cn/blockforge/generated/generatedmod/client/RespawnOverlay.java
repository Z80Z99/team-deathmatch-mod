...
public final class RespawnOverlay {
    ...
    public static void render(ForgeGui gui, GuiGraphics graphics, float partial, int width, int height) {
        ...
        graphics.fill(left, top, left + 24, top + 2, color((int) (255 * fade), 0xE65861));
        graphics.drawString(mc.font, "已阵亡", left, top + 10, white, false);
        ...
        String status = elimination ? "本回合已阵亡 · 等待下一回合"
                : remaining > 0 ? "重新部署  " + (remaining / 20 + (remaining % 20 == 0 ? 0 : 1)) + " 秒"
                : "正在寻找安全复活位置";
        ...
        if (!elimination) {
            ...
            int filled = (int) (panelWidth * progress);
